...
public class MonitoringWindowTests
{
    ...
    public void Constructor_KeepsTheWindowFromBeingShrunkBelowItsContent()
    {
        ...
        // The window may never be resized below the width its rows need, otherwise a control would be
        // pushed out of sight (the autostart checkbox and the bind button used to disappear that way).
        Assert.True(window.MinimumSize.Width >= root.PreferredSize.Width);
        Assert.True(window.MinimumSize.Height >= 500);
    }
